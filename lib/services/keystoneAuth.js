...
function authenticate(req, res, next) {
    if (authenticating) {
        waitingRequests.on('token', domainContinuator(domainModule.active, next));
        ...
        logger.debug('Authenticating PEP Proxy against Keystone: %j', optionsauthenticate);
        ...
        // Revert to see if increase cpu and mem
        // next();
    } else {
        next();
    }
...
function authenticationProcess(req, res, next) {
    const retries = config.authentication.retries || 3;
    var attempts = 0;
