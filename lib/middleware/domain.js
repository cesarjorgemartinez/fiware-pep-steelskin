...
function requestDomain() {
    ...
    return function requestDomain(req, res, next) {
        ...
        domainErrorHandler = function(err) {
            ...
            delete reqDomain.start;
            delete reqDomain.corr;
            delete reqDomain.msg;
            // next(err);
            next();
            // reqDomain.exit();
        };
        ...
        requestHandler = function() {
            ...
            if (corr) {
                ...
                req.corr = reqDomain.corr; // for propagate in FWD request
            }
            logger.debug('using correlator: %s', reqDomain.corr);
            res.set(constants.CORRELATOR_HEADER, reqDomain.corr); // for response
