...
function initializeProxy(proxyObj, callback) {
    ...
    proxyObj.proxy.use(bodyParser.json({limit: config.bodyLimit ? config.bodyLimit : '1Mb'}));
    ...
    validation.init(function initValidation(loadingError) {
        if (loadingError) {
            logger.error('Error initValidation: ' + loadingError.message);
        } else {
            proxyObj.server.listen(
                proxyObj.proxy.get('port'),
                proxyObj.proxy.get('host'),
    ...
    validation.init(function initValidation(loadingError) {
        if (loadingError) {
            ...
            proxyObj.adminServer.listen(
                ...
                proxyObj.administration.get('host'),
                function startServer(error) {
                    if (error) {
